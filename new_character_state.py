...
class NewCharacterState(IAppState):
    ...
    def start(self):  # called when this state first appears
        ...
        return_to_player_menu_button_rect = pygame.Rect(0, 0, 150, 40)
        return_to_player_menu_button_rect.centerx = self.window_surface.get_rect().centerx
        return_to_player_menu_button_rect.top = self.window_surface.get_height() * 0.8
        self.return_to_player_menu_button = UIButton(relative_rect=return_to_player_menu_button_rect,
    ...
    def process_event(self, event: pygame.event.Event):  # takes event as parameter
        ...
        if event.type == UI_BUTTON_PRESSED:  # if a button is pressed
            if event.ui_element == self.return_to_player_menu_button:  # if return to player menu pressed
                ...
                print('RETURN PLAYER MENU button pressed in NEWCHARACTERSTATE')
